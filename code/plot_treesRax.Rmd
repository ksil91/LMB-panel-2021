...
```{r}
tree <- read.tree("../analyses/DBout_mac3het7p4R7.min112.BS.treefile")
MRCA(tree, c("SHB_UFW17868","GLB_10702","TNSPB_04","REB_HatS214","SMB_BUR20") )
```
...
```{r}
MRCA(tree,tip=c("GLB12050","GLB12048"))
MRCA(tree,tip=c("GASHB24","GASHB23","SHBUSFWS17868fastq"))
MRCA(tree,tip=c("TNSPB05","TNSPB04"))
MRCA(tree,tip=c(tree$tip.label[grep("CAN",tree$tip.label)],tree$tip.label[grep("DB",tree$tip.label)],tree$tip.label[grep("TN1",tree$tip.label)]))
MRCA(tree,tip=tree$tip.label[grep("SL",tree$tip.label)])
```
...
p2 <- facet_plot(p, panel="Q-values", data = y, geom=geom_barh, 
                 mapping= aes(x=mean_prob,fill=Lineage),stat='identity',width=1) +theme_tree2(legend.position="left") +
  ...
  geom_cladelabel(node=155, label="Guadalupe Bass", align=F,fontsize=4) +
  geom_cladelabel(node=149, label="Shoal Bass", align=F,fontsize=4) +
  geom_cladelabel(node=154, label="Spotted Bass", align=F,fontsize=4) 
